...
- 最大 4 枚を 2×2 で**並べて見る**
- 配信中を**視聴が多い順**に一覧できる
- **休憩モード** — 3/5/10 分を選ぶと、行き先はアプリが選ぶ。時間が来たら静かに終わる
- 日本語 / English

## 休憩モード

仕事や勉強の合間に開くアプリなので、**休むときに何も決めさせない**。3・5・10 分の
どれかを押すだけで、行き先はアプリが選ぶ。選定の芯は「**見る人と逆の時間帯へ連れて行く**」
こと — 夜に働いているなら昼の街へ、昼なら夜の街へ。地図に引いてある昼夜の境界を、
そのまま気分転換の意味に使っている。

時間が来たら静かに終わる。音は鳴らさず、映像を止めて「おつかれさま」とどこへ行ったかだけ
出す。**音はどこでも既定でミュート**で、本人が明示的に出したときだけ鳴る（職場で開いて
音が出る事故を避けるため）。この選択と、直近に行った場所は localStorage に覚えておく。
...
| | 単価 | 頻度 | 日あたり | 枠 |
|---|---|---|---|---|
| 生存確認 `videos.list` | 1 unit / 50 件 | ライブは 2 時間・それ以外は 20 分ごと | 約 3,500 | 4,000 |
| 再探索（uploads を辿る） | 2〜6 units / チャンネル | 毎時・呼び出し 40 回に収まるだけ | 0〜2,000 | 4,000 |
| 再探索（検索へ後退） | 101 units | 毎時 1 回まで | 0〜2,424 | （同上） |
| | | **合計** | | **8,000** |

**枠は役割ごとに分ける。** ひとつの財布を先着順で使うと、10 分ごとに回る生存確認が
1 日ぶんを食い尽くし、1 時間に 1 度しか来ない再探索が兵糧攻めになる（実測で生存確認が
全消費の 7 割を占め、日の終わりには枠が尽きて更新が止まっていた）。台帳も
`quota-ledger:sweep` / `quota-ledger:rediscover` に分けてある — 2 つの Cron は毎正時に
**同時に**起きるので、ひとつの台帳を読んで書くと後勝ちで一方の消費が丸ごと消える。

**全件を同じ頻度では確認しない。** ライブカメラのほとんどは 24 時間流しっぱなしで、
いちど live と分かった配信はそうそう変わらない。変化が起きるのは offline / blocked の側
なので、そちらだけを短い間隔で回す（`RECHECK_INTERVAL_MS`）。引き換えに配信が終わった
ことに気づくのが最大 2 時間遅れるが、再生できない配信を掴んだブラウザは自分で blocked
に落とすので、見ている人の画面はそこまで待たされない。
...
検索（101 units）に後退する。後退は 1 回の実行で 1 チャンネルまで。生存確認の方が先で、
再探索は「記録した videoId が死んだ」ときのためのもの。

> ⚠️ **一巡の速さを決めているのはクォータではなく、Workers のサブリクエスト上限（50）。**
> チャンネル 1 本は uploads を最大 3 ページ辿って 6 回呼ぶので、1 回の実行で回せるのは
> せいぜい 6〜19 本。件数だけを増やすと落ちる（`MAX_CALLS_PER_REDISCOVER`）。
> これより速くしたいなら、1 回の本数ではなく **Cron の頻度**を上げる方が筋がよい。
